...
| Event | Type | Purpose |
|---|---|---|
| `SessionStart` | command | Loads `wiki/hot.md` into context via `cat wiki/hot.md`. Always exits 0 so non-vault sessions are safe. Matcher: `startup\|resume`. Prompt-type hooks are NOT supported for SessionStart (no conversation context available at that point). |
