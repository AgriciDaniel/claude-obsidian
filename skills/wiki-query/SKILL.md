...
- **cli** — `"$CLI" read vault="$VAULT" path="$NOTE"` and `"$CLI" search vault="$VAULT" query="<query>"` (Obsidian-native ranking). Args are `key=value`; `$VAULT` is the vault NAME from `available.cli.vault_name`. See [`skills/wiki-cli/SKILL.md`](../wiki-cli/SKILL.md)
