...
Use this as a safe draft for the official application form. Confirm the GitHub account, organization ID, and maintainer role before submitting.
...
Recommended answer if this is submitted by the repository owner or active maintainer:

```text
Primary maintainer
```

If submitted by a collaborator without repository ownership, use `Core maintainer` only if the account has clear maintainer responsibility and can verify it.
...
- Confirm the submitting ChatGPT email.
- Confirm the GitHub username is public.
- Confirm the repository is public.
- Confirm the submitter can truthfully claim primary or core maintainer status.
- Confirm the OpenAI Organization ID from the Platform dashboard.
- Do not attach private vault files or screenshots with personal data.
