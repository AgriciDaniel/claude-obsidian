...
If you find a security issue in Compound Vault, please report it privately rather than opening a public issue.

**Preferred:** GitHub's private reporting at the repository's [Security Advisories](../../security/advisories/new) page.

**Alternative:** Email **agricidaniel@gmail.com** with subject line `Compound Vault security`.
...
Compound Vault assumes a **single-tenant** deployment: one user, one vault, one machine. Several design decisions follow from this assumption and would need explicit hardening for multi-tenant or shared-CI scenarios:
...
- **The PostToolUse auto-commit hook** (`hooks/hooks.json`) runs as the local user invoking the agent. It auto-commits `wiki/`, `.raw/`, and `.vault-meta/` paths to the local repo on every Write/Edit. Set `.vault-meta/auto-commit.disabled` (any contents) to opt out per-vault. For shared repos, prefer disabling the hook entirely or using a more restrictive commit policy.
