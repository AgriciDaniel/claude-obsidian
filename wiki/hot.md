...
2026-07-04: **Vault housekeeping session: 3 new Obsidian plugins enabled, MCP transport configured, hot cache backfilled.** Follow-up to the 2026-05-28 catch-up entry below. Actions taken: (1) Enabled `dataview`, `obsidian-git`, `templater-obsidian` in `.obsidian/community-plugins.json` (these had been downloaded into `.obsidian/plugins/` but not yet added to the enabled list); Templater's `templates_folder` pre-set to `_templates` via a new `data.json` (gitignored, host-local per existing calendar/thino convention). Obsidian Git left at plugin defaults (user should set auto-backup interval manually in Settings if wanted, since it changes commit behavior). Committed as `4f18efd`, local-only, not pushed. (2) Configured MCP transport: `obsidian-cli` binary not found despite Obsidian 1.12.7 being installed, so used **Option B (MCPVault)** from `skills/wiki/references/mcp-setup.md` instead of Option A/D — no Local REST API plugin dependency, no TLS bypass. Registered via `claude mcp add-json obsidian-vault ... --scope user` pointing at this vault path; confirmed `✔ Connected` and verified live with `mcp__obsidian-vault__get_vault_stats` (138 notes, 59 folders, correctly surfaced `wiki/log.md`/`wiki/hot.md` as most recently modified). Updated `.vault-meta/transport.json` (gitignored, host-local) to `preferred: mcpvault`, `manual_override: true`, since `scripts/detect-transport.sh` doesn't auto-detect MCP servers per its own design (v1.7 limitation noted in the transport doc). (3) Left `.claude/` directory untracked as-is (local session config: `settings.local.json` + 4 command shortcuts `wiki.md`/`canvas.md`/`save.md`/`autoresearch.md`) — not part of the public plugin repo surface.

2026-05-28: **v1.9.2 promoted to public canonical + social preview asset shipped.** Three commits since the last hot.md checkpoint (which had drifted ~10 days behind `git log`): (1) `73616fa` v1.9.2 feat — prompt-cache hardening on `contextual-prefix.py` (cache_control only attached above the Haiku 4.5 floor, `HAIKU_CACHE_MIN_CHARS=16384`, extracted as pure `cache_control_for()`; per-call cache telemetry logs token counts only, never page content, preserving the v1.7.1 data-egress posture) plus path-handling fixes (missing page path now exits 3 instead of silently exiting 0; out-of-vault path exits 2 with a message instead of a raw traceback); `make test` at 9 suites green. (2) `00213b7` release commit — flips README/docs framing so the public repo (AgriciDaniel/claude-obsidian) is the default install everywhere, repositions "AI Marketing Hub Pro" as early-access rather than default, corrects the install slug to `claude-obsidian@agricidaniel-claude-obsidian` (verified against `claude plugin list`), adds SSS+ compliance files (CITATION.cff, PRIVACY.md, CODEOWNERS, FUNDING.yml), and runs an SEO/GEO pass on the README (4 verbatim GEO Q&As added to FAQ for AI Overview / Perplexity citation). Gates: secret scan clean over 55-commit history, `make test` 9/9, `plugin validate` clean, verifier agent SHIP (0 BLOCKER/0 HIGH). (3) `cb93ff6` chore — adds `assets/social-preview.png` (1280x640 GitHub social card). **Plugin version is now 1.9.2, publicly canonical.** No further local-only branches pending push as of this entry (previous v1.7.0-compound-vault branch concerns from the 2026-05-17 entries below are superseded — work has since landed on main through v1.9.2).
...
- **Version**: 1.9.2 (public canonical; plugin.json + marketplace.json synced; landed on main, no pending local-only branch)
...
- **Tests**: `make test` runs 9 suites — `test_allocate_address.sh`, `test_tiling_check.py`, `test_boundary_score.py`, `test_bm25_index.py`, `test_retrieve.py`, `test_wiki_lock.sh`, `test_concurrent_write.sh`, **`test_mode.py` (v1.8)**, **`test_contextual.py` (v1.9)**. Zero ollama and zero network dependency for all core tests.
