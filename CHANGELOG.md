...
Three defects that all shared one root cause — **the plugin was never actually exercised end-to-end on macOS**. The Obsidian CLI transport silently never engaged, and a third of the test suite could not run at all. Every recipe and fix below was executed against a live vault, not reasoned about.
...
`make test` now passes 10/10 on macOS — the first time the suite has been green on the platform.
...
- `scripts/lib/portable-lock.sh` — the portable exclusive-lock primitive described above, plus `tests/test_portable_lock.sh` and a `make test-portable-lock` target (`make test` is now 10 suites).
