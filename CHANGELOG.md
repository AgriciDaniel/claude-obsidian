...
## [Unreleased] (macOS portability: CLI transport + flock)

Six defects that all shared one root cause: **the plugin was never actually exercised end-to-end on macOS, and CI only ever ran on `ubuntu-latest`.** The Obsidian CLI transport silently never engaged, a third of the test suite could not run at all, and none of the documented CLI recipes were executable as written.

Two of the six are the *same* failure — a component reporting success while targeting the wrong thing. The transport fell back to `filesystem` while claiming `cli`; the CLI then read and wrote a different directory entirely while claiming to write this one. Every fix below was found by running the code, not by reading it.

### Fixed

- **The Obsidian CLI transport never engaged on macOS** (`scripts/detect-transport.sh`). Detection probed only `PATH`, but Obsidian 1.12 ships the CLI *inside* the app bundle and does not symlink it out. Every stock macOS install therefore fell through to the `filesystem` floor while reporting success — the exact silent-downgrade failure mode the transport design was meant to prevent. Detection now walks a candidate list (`PATH` first, then `/Applications/Obsidian.app/Contents/MacOS/obsidian-cli` and the `~/Applications` variant) and validates each with a capability probe. Verified to still resolve `cli` when a `PATH` symlink dangles — the real state on a Gatekeeper-translocated Obsidian, where the symlink target is randomized per launch.
- **The version probe could never have succeeded** (`scripts/detect-transport.sh`). It invoked `obsidian-cli --version`; the CLI accepts only the `version` subcommand and rejects the flag.
- **`flock(1)` is util-linux and does not exist on macOS** (`scripts/allocate-address.sh`, `scripts/wiki-lock.sh`). Both shell scripts called it directly, so address allocation and *every advisory lock in the vault* failed hard on any Mac, taking 3 of 9 `make test` targets red. Locking now goes through `scripts/lib/portable-lock.sh`, which takes the same `flock(2)` kernel lock on fd 9 by either route: `flock(1)` where it exists (unchanged behavior on Linux), and `python3`'s `fcntl.flock` where it does not. The python path works because `flock(2)` locks the *open file description*, not the process — python3 locks the inherited fd and exits, the shell keeps fd 9 open, and the kernel releases the lock when the shell exits. That is exactly what `flock(1)` does internally. Python code was never affected: `fcntl.flock` is a POSIX syscall and works on macOS.
- **BSD `wc -l` pads its output with leading spaces**, so bare string comparisons against it failed on macOS (`tests/test_allocate_address.sh`, `tests/test_wiki_lock.sh`, `tests/test_concurrent_write.sh`).
- **The CLI could silently read and write a completely different directory** (`scripts/detect-transport.sh`). `vault=<name>` is resolved against Obsidian's own vault registry, never against the working directory — so a present, working binary does **not** mean it can reach the vault you are standing in. If Obsidian has another folder registered under the same name (a stray clone in `~/Downloads`, say), every CLI call targets *that* folder and reports success. Detection now asks the CLI which vaults it knows, matches one against the vault root, and refuses to prefer `cli` unless a vault is genuinely addressable — falling back to `filesystem` with an actionable warning. The verified name is published as `available.cli.vault_name` so consumers pass a checked `vault=` instead of guessing from the directory basename, and `skills/wiki-cli/SKILL.md` now documents the footgun. This is the same silent-wrong-target failure as the transport bug above, in a second location: found when it swallowed a whole session's CLI writes while the real working tree sat untouched.
- **The CLI capability probe could hang detection indefinitely** (`scripts/detect-transport.sh`). Probing a candidate with `version` assumed a non-CLI binary would exit cleanly, but an Electron launcher special-cases `--version` and may treat a bare `version` as a file to open — leaving a window up and the probe blocked. Since `detect-transport.sh` is invoked from several skills, that would wedge them. The probe is now bounded by a hand-rolled 3s timeout (macOS ships no `timeout(1)`; it is GNU coreutils), degrading a hang into a failed probe that falls through to the next candidate. Verified by putting a deliberately hanging `obsidian-cli` on `PATH`: detection completes in 3s instead of blocking.

`make test` now passes 11/11 on macOS — the first time the suite has been green on the platform.

A note on how the lock got here, because it is the most load-bearing change in the release. The first attempt emulated `flock` in pure shell with an atomic `mkdir` spin plus stale-lock reaping. It looked right, and the pre-commit verifier caught a real use-after-steal defect in it. The *fixed* version then still failed a brute-force contention test — two workers entered the critical section together, roughly one run in five. The root cause is that macOS ships bash 3.2, which has no `BASHPID`, so a subshell cannot even identify itself; every reaping scheme built on that foundation leaks a way for two holders to enter at once. Rather than patch a third time, the implementation was replaced with a real kernel lock, which deletes the entire class of bug: no reaping, no PID tracking, no owner tokens, no `EXIT` trap, no steal semantics. `tests/test_portable_lock.sh` asserts mutual exclusion under 6 × 20 contended workers and was run 8× (960 contended acquisitions, 0 violations); it also covers timeout, release-on-exit, and reclaim-after-`SIGKILL`.

### Changed

- **Every recipe in `skills/wiki-cli/SKILL.md` was wrong and none of them ran.** The CLI takes `key=value` options, not positional arguments. Beyond syntax: `vault=` takes a vault *name*, not a path; there is no `write` command (`create` + `overwrite` is the upsert); `daily:today` does not exist (`daily:read` / `daily:path`); and `content=` is a shell argument with `\n` escapes, not stdin — so the filesystem Write tool is now documented as the better choice for anything longer than a few lines. All 18 rewritten recipes were executed against a live vault before shipping.
- `available.cli.binary` in `transport.json` may now hold an absolute path rather than a bare command name. Consumers must exec it verbatim instead of assuming it is `PATH`-resolvable; `skills/wiki-cli/SKILL.md` documents resolving it from the snapshot. No existing consumer reads this field — the four skills that parse `transport.json` only read `preferred`.

### Added

- `scripts/lib/portable-lock.sh` — the portable exclusive-lock primitive described above, plus `tests/test_portable_lock.sh` and a `make test-portable-lock` target.
- `tests/test_detect_transport.sh` + `make test-transport` — hermetic vault-addressability tests using a mock CLI, pinning the guard above (including the case where the binary works perfectly but the only registered vault lives somewhere else). `make test` is now 11 suites.
- **A macOS CI job** (`.github/workflows/test.yml`). Every defect in this release was a macOS portability bug that shipped because CI only ever ran on `ubuntu-latest` — missing `flock(1)`, BSD `wc -l` padding, no `timeout(1)`, and bash 3.2 with no `BASHPID`. A Linux-only matrix is structurally incapable of catching any of them. This job is the regression guard for the whole class, and is the single most valuable change in the release.
- **`PORTABLE_LOCK_FORCE_BACKEND=flock|python3`** plus a CI step that runs the lock suite under a forced `python3` backend. Linux runners always have `flock(1)`, so without this the python path would have zero automated coverage on any platform. The test suite asserts the forced backend was *actually* taken (the library reports it via `PORTABLE_LOCK_LAST_BACKEND`) — an earlier version of this check silently ran `flock` while claiming to test python, which is worse than no check at all.
- **An fd-9 nesting guard** in `portable_lock_acquire`. Two locks in one shell scope would silently drop the first when fd 9 was reopened. It now fails loudly rather than losing a lock without a word.
- Two CLI behaviors documented as traps in `skills/wiki-cli/SKILL.md`, both found only by running the commands: `base:views` silently ignores `path=` and reads whatever file is active in the GUI (it is not scriptable), and `base:query` without `view=` resolves the *first* view rather than all of them.
- Recipes for the CLI's native `orphans` / `unresolved` / `deadends` commands, which `wiki-lint` currently reimplements by hand.
